...
3. **竞品代理硬性排除**：明确代理 Mako / ROSA / Cori 等竞品手术机器人的经销商，因与我方存在直接渠道冲突，一律判定为 `rejected`（见第 7 步硬性排除），不进入评分。
...
7. **硬性排除对象**：以下几类无论评分多高，一律判定为 `rejected`，不得标记为 `qualified`/`new`/`human_review`，也不得通过 `add_leads` 保存或出现在最终报告中。这是判断题，不是打分题——命中即拒绝，不需要先算分再看阈值：
   ...
   - **竞品机器人代理商**：官网 / 新闻 / LinkedIn 明确显示代理 Mako / ROSA / Cori 等竞品手术机器人的经销商——存在直接渠道冲突，一律 `rejected`，不受评分高低影响。
