...
def _get_tavily():
    ...
    if _TAVILY_API_KEY and _TAVILY_CLIENT is None:
        try:
...
def search_web(query: str, *, http: HttpClient | None = None, limit: int = 8) -> list[SearchResult]:
    # Tavily first (API-based, works from China)
    results = search_tavily(query, limit=limit)
    if results:
        return results
...
def search_tavily(query: str, *, limit: int = 8) -> list[SearchResult]:
    """Search via Tavily API — works from mainland China, returns high-quality English results."""
    client = _get_tavily()
    if not client:
        return []
    ...
    for item in response.get("results", []):
        ...
        if not url or not _is_http_url(url):
            continue
        if _is_low_value_domain(_domain(url)):
            continue
        results.append(
            SearchResult(
                title=unescape(title) if title else url,
                url=url,
