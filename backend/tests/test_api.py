...
def _auth_headers() -> dict[str, str]:
    return {"Authorization": f"Bearer {_TEST_SERVICE_TOKEN}"}


def _auth_get(client: TestClient, path: str, **kwargs):
    """GET with auth headers."""
    kwargs.setdefault("headers", _auth_headers())
    return client.get(path, **kwargs)


def _auth_post(client: TestClient, path: str, **kwargs):
    """POST with auth headers."""
    kwargs.setdefault("headers", _auth_headers())
    return client.post(path, **kwargs)


def _auth_patch(client: TestClient, path: str, **kwargs):
    """PATCH with auth headers."""
    kwargs.setdefault("headers", _auth_headers())
    return client.patch(path, **kwargs)


def _auth_put(client: TestClient, path: str, **kwargs):
    """PUT with auth headers."""
    kwargs.setdefault("headers", _auth_headers())
    return client.put(path, **kwargs)


def test_health_check_no_auth_required(tmp_path, monkeypatch):
...
def test_auth_login_success(tmp_path, monkeypatch):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["username"] == "microport_admin"
    assert data["token_type"] == "bearer"
    assert len(data["access_token"]) > 20


def test_auth_login_bad_credentials(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as client:
        response = client.post(
            "/auth/login",
            json={"username": "microport_admin", "password": "wrong"},
        )
...
def test_auth_verify_with_valid_token(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as client:
    ...
    assert response.status_code == 200
    assert response.json()["username"] == "microport_admin"
    assert response.json()["valid"] is True


def test_protected_route_returns_401_without_token(tmp_path, monkeypatch):
    monkeypatch.setenv("MEDBOT_DB_PATH", str(tmp_path / "medbot-demo.db"))
