...
不发：终端医院作为直接采购方机构（区别于医院内具名 KOL，那些正常按 kol 评估）、媒体、咨询公司、工业机器人公司（非医疗）、纯科研机构。这是 Agent 判断题，命中即拒绝，不受评分高低影响——由提示词（SKILL.md）驱动，不做代码硬编码过滤，因为这类判断依赖对搜索证据的语义理解。
**变更：** 代理竞品（Mako / ROSA / Cori）者**不属于**硬性排除，改为个案评估、评分中酌情降级。
