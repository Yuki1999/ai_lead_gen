...
实现：回复意向分析为 **LLM 纯语义判断**（无关键词兜底）。见 `backend/app/services.py` 的 `analyze_reply` —— 其提示词中列全上述 8 类"转人工"主题，命中任一即置 `requires_human=true`，按语义判断（多语言、改写措辞均可覆盖），不依赖字面关键词匹配。

## 评分权重（业务信息表）

渠道匹配度 40% · 目标市场战略优先级 25% · 学术/品牌公开资历 20% · 联系方式可用性 10% · 公开 KOL/医院合作记录 5%（加分）。

## 排除对象（硬性，见 SKILL.md 第 0.7 节）

不发：终端医院作为直接采购方机构（区别于医院内具名 KOL，那些正常按 kol 评估）、媒体、咨询公司、工业机器人公司（非医疗）、纯科研机构、**明确代理竞品手术机器人（Mako / ROSA / Cori 等）的经销商**（直接渠道冲突）。这是 Agent 判断题，命中即拒绝，不受评分高低影响——由提示词（SKILL.md）驱动，不做代码硬编码过滤，因为这类判断依赖对搜索证据的语义理解。
